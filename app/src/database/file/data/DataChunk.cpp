...
const std::vector<uint8_t>& DataChunk::getData() const {
    return data;
}
...
std::streampos Serialize<DataChunk>::toStream(const DataChunk& dataChunk, std::streampos position, std::fstream& fileStream) {
    ...
    std::array<uint8_t, 4> rawDataChunkLength = { UINT32_TO_UINT8(dataChunk.getData().size()) };
    ...
    fileStream.write(reinterpret_cast<const char *>(dataChunk.getData().data()), dataChunk.getData().size());
