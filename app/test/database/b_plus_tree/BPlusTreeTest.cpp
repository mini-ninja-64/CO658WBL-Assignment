...
// BPlusTree<ORDER, K, K> createTreeWithKeyAsValue(std::initializer_list<K>
// insertions) {
...
//     ASSERT_EQ(uncheckedInternalNode(root->getChildren()[0])->getChildren().size(),
//     2);
//
//     ASSERT_EQ(root->getChildren()[1]->getRecords().size(), 1);
//     ASSERT_EQ(uncheckedInternalNode(root->getChildren()[1])->getChildren().size(),
//     2);
// }
//
...
//     auto testTree = createTreeWithKeyAsValue<4, int>({0, 1, 2, 3, 4, 5, 6, 7,
...
//     auto testTree = createTreeWithKeyAsValue<4,
//     int>({3,10,0,5,4,11,1,6,2,14,7,8,15,9,13,12});
//
//     auto root = uncheckedInternalNode(testTree.getRoot());
