...
class FileBackedBPlusTree {
private:
    IndexFile<K, ADDRESS> indexFile;
    DataFile<ADDRESS> dataFile;

private:
    std::tuple<LazyNode<K,ADDRESS>, LazyNode<K,ADDRESS>> findParentAndLeaf(const K& key) {
        ...
        do {
            LazyNode<K, ADDRESS> child = std::dynamic_pointer_cast<FileBackedInternal<K,ADDRESS>>(parent.get())->next(key);
            if(child.get()->getNodeType() == NodeType::Leaf) leaf = child;
            else parent = child;
        } while (!leaf);
    ...
    std::string recurseChildrenGraphViz(LazyNode<K,ADDRESS>& node) {
        ...
        if(node.get()->getNodeType() == NodeType::Leaf) {
            auto dataAddresses = std::static_pointer_cast<FileBackedLeaf<K, ADDRESS>>(node.get())->getDataAddresses();
        ...
        if(node.get()->getParent()) {
            auto parentNodeName = "node" + std::to_string(node.get()->getParent().value().getAddress());
            graphVizCode += nodeName + " -> " + parentNodeName + "[color=\"blue\"]\n";
        }

        if (node.get()->getNodeType() == NodeType::Internal) {
            auto internalNode = std::static_pointer_cast<FileBackedInternal<K, ADDRESS>>(node.get());
    ...
    void addChildToParent(LazyNode<K, ADDRESS> parentNode, LazyNode<K, ADDRESS> childNode, const K& propagatedKey) {
        if(parentNode.get()->getNodeType() != NodeType::Internal) throw std::domain_error("Only an internal node can be a parent node");
        auto parentNodePointer = std::static_pointer_cast<FileBackedInternal<K, ADDRESS>>(parentNode.get());
        ...
        if(parentNodePointer->getRecords().size() >= indexFile.getMetadata().graphOrder) {
            ...
            if(parentNode == getRoot()) {
                ...
                indexFile.saveNode(parentNode.getAddress(), std::move(parentNodePointer));
                indexFile.saveNode(newRightParent.getAddress(), std::move(rightParentPointer));
            } else {
...
public:
    ...
    void insert(const K& key,  DataChunk dataChunk) {
        ...
        auto leafPointer = std::static_pointer_cast<FileBackedLeaf<K, ADDRESS>>(leaf.get());
        leafPointer->insertOrdered(key, newData.getAddress());

        if(leafPointer->getRecords().size() >= indexFile.getMetadata().graphOrder) {
            const auto leftNodeSize = indexFile.getMetadata().graphOrder/2;
    ...
    LazyNode<K, ADDRESS> getRoot() {
        return indexFile.getRootNode();
    }
    ...
    std::optional<LazyDataChunk<ADDRESS>> find(K key) {
        ...
        if (recordIndex) {
            auto leafNodePointer = std::static_pointer_cast<FileBackedLeaf<K, ADDRESS>>(leaf.get());
            auto dataAddress = leafNodePointer->getDataAddresses()[recordIndex.value()];
