...
private:
  ...
  void writeMetadata() {
    ...
    Serialize<IndexMetadata<ADDRESS>>::toStream(metadata, metadataPosition,
                                                file);
  }

  NodePointer pullNodeFromFile(ADDRESS address) {
    auto node = Deserialize<std::unique_ptr<FileBackedNode<K, ADDRESS>>,
                            DeserializeGraphContext<K, ADDRESS>>::
        fromStream(address, file,
...
public:
  ...
  IndexFile(const std::filesystem::path &filePath, bool forceOverwrite,
            uint32_t defaultOrder)
      : metadata({.graphOrder = defaultOrder,
                  .numberOfNodes = 0,
                  // By default, the root node will be the first node after the
                  // metadata
    ...
    if (write) {
      ...
      auto indexPointer =
          Serialize<DatabaseFileHeader>::toStream(indexHeader, 0, file);
  ...
  LazyNode<K, ADDRESS> insertNode(NodePointer node) {
    ...
    insertionPosition =
        Serialize<FileBackedNode<K, ADDRESS>, SerializeGraphContext>::toStream(
            *node, nodeAddress, file, {.order = metadata.graphOrder});
  ...
  void saveNode(ADDRESS address, NodePointer node) {
    Serialize<FileBackedNode<K, ADDRESS>, SerializeGraphContext>::toStream(
        *node, address, file, {.order = metadata.graphOrder});
  ...
  LazyNode<K, ADDRESS> getRootNode() { return {this, metadata.rootNode}; }

  const IndexMetadata<ADDRESS> &getMetadata() const { return metadata; }
