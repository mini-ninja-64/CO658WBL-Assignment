...
template <typename T, typename CONTEXT = void> struct Deserialize {
  static T fromStream(std::streampos position, std::fstream &fileStream,
                      CONTEXT context);
};

template <typename T> struct Deserialize<T, void> {
  static std::streampos toStream(const T &element, std::streampos position,
