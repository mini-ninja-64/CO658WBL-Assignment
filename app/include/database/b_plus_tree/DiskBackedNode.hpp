...
//
//    explicit DiskBackedNode(const std::vector<KEY_TYPE> &records) : records(records) {}
//
...
//    return  1                                               + /* node type */
//            4                                               + /* number of records */
//            deserializedKeySizeInBytes<KEY_TYPE>() * order  + /* records */
//            nodePointerSize * (order + 1)                   + /* pointers to children */
//            nodePointerSize                                 + /* previous leaf */
//            nodePointerSize;                                  /* next leaf */
//}
//
//template<typename KEY_TYPE, typename VALUE_TYPE>
//DiskBackedNode<KEY_TYPE, VALUE_TYPE>* fromBuffer(size_t order, std::span<uint8_t> buffer) {
//    const auto nodeSize = sizeInBytesOfNode<KEY_TYPE>(order);
//    if(buffer.size() < nodeSize)
//        throw std::length_error("provided buffer not large enough to contain node");
//
