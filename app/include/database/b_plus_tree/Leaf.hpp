...
//
//
//     explicit Leaf(const std::vector<KEY_TYPE> &records, const
//     std::vector<VALUE_TYPE> &values) : Node<KEY_TYPE, VALUE_TYPE>(records),
//     values(values) {}
//
// public:
...
//         if(!recordIndex.has_value()) return std::nullopt; // Key is not
//         present, nothing to do except return early
//
...
//         this->records.end()); this->values.erase(this->values.begin() +
//         splitIndex, this->values.end());
//
...
//         this->records.begin()+splitIndex);
//         this->values.erase(this->values.begin(),
//         this->values.begin()+splitIndex);
//
