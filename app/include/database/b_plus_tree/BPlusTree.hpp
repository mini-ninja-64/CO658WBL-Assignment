...
//     size_t nodeCount = 1;
//
//     std::tuple<Internal<K, V>*, Leaf<K, V>*> findParentAndLeaf(const K& key)
//     const {
...
//             if(child->getNodeType() == NodeType::Leaf) leaf =
//             dynamic_cast<Leaf<K, V>*>(child); else parent =
//             dynamic_cast<Internal<K, V>*>(child);
//         } while (leaf == nullptr);
//
...
//             for (auto& child :
//             dynamic_cast<Internal<K,V>*>(node)->getChildren()) {
//                 std::string childNodeName = "childNode" +
//                 std::to_string(currentIndex); currentIndex++; graphVizCode +=
//                 recurseChildrenGraphViz(childNodeName, child, currentIndex);
...
//     }
//
//     void addChildToParent(Internal<K, V>* parentNode, Node<K, V>*
//     childNode,const K& propagatedKey) {
...
//                 root = new Internal<K, V>(root, keyToPropagate,
//                 newRightParent); nodeCount+=2;
//             } else {
//                 auto keyToPropagate = parentNode->getRecords()[leftSize];
//                 auto newRightParent = parentNode->splitRight(leftSize);
//                 nodeCount+=1;
...
//                     throw std::invalid_argument("A node with an invalid
//                     parent type was supplied, parents can only be internal");
//                 addChildToParent(dynamic_cast<Internal<K, V>*>(nextParent),
//                 newRightParent, keyToPropagate);
//             }
//         }
//     }
//
...
//     BPlusTree(const size_t order, Node<K, V> *root, size_t nodeCount) :
...
//
//             if(parent == nullptr) {
//                 // This means we are splitting the root node as there is no
//                 parent,
//                 // the cleanest way to carry this out is to create a new leaf
//                 to the right
//                 // then replace the existing root which is a leaf, with a new
//                 internal node
//                 // that utilises the old root as a leaf child.
//                 auto newLeaf = dynamic_cast<Leaf<K,
//                 V>*>(root)->splitRight(leftSize); root = new Internal<K,
//                 V>(root, newLeaf->getRecords()[0], newLeaf); nodeCount+=2;
//             } else {
//                 // This means we must split the leaf to the right and carry
//                 out the standard
//                 // insertion procedure of a B+ Tree, which in this case is
//                 handled recursively
...
//         if (recordIndex.has_value()) return
//         leaf->getValues()[recordIndex.value()]; return std::nullopt;
//     }
//
...
//         // Note: Some B+ tree implementations choose to propagate upwards any
//         key removals,
//         //       this is an interesting choice and in my opinion does not
//         serve much purpose,
//         //       since the internal nodes are only used for searching,
//         aligning a non-broken
//         //       index won't improve search performance, but does cost budget
//         to carry out.
//         //       (at least with my current understanding)
//
//         // Handling Tree Underflow
//         // Note: Some B+ tree implementations (including this one) choose to
//         ignore tree underflow's,
//         //       the argument being that in actual systems the amount of
//         inbound data far outweighs
//         //       the number of deletions so anything deleted will likely be
